...
function MathEditor({
  ...
  if (showLatexHelp === undefined) showLatexHelp = true;
  if (showInsertButtons === undefined) showInsertButtons = true;
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-row z-20 gap-1 rounded-t-md border border-b-0 p-1 bg-gray-100 font-sans ${mode === "preview" ? "border-t rounded-t-md" : `${showInsertButtons && "group-focus-within:border-t-0 group-focus-within:rounded-t-none"}`}`}
      >
        <button
      ...
      {mode === "edit" ? (
        <textarea
          className={cn(
            `z-20 min-h-24 w-full rounded-md rounded-t-none border bg-white p-2 text-sm ${showLatexHelp && "focus:rounded-b-none"} focus:outline-none`,
            className,
          )}
